<template>
  <PageRoot>
    <view class="supplies-page">
      ...
      <view v-else-if="list.length === 0" class="status-tip">还没有水果，先添加一件吧</view>
    ...
    <view v-if="modalOpen" class="modal-panel" @tap.stop @touchmove.stop>
      <text class="modal-title">{{ editingId ? '编辑' : '新增' }}水果</text>
